...
void countburden(leaf *right, leaf *left, const leaf *src){
	...
	if (left) {
		...
		left->burden = src->burden - cross - right->burden;
	}
...
double split_score(leaf *lf, label *lab, graph &g) {
    ...
    uint32_t n_right = lab->nodecnt;
    uint32_t n_left = lf->nodecnt - n_right;
    uint32_t cross_edges = lab->crosscnt;
    return -(1-lambda)* n_right *n_left +cross_edges;
}
...
/************** read sizes **********/
void read_sizes(graph & g, FILE *lf, FILE *ef)
{
	uint32_t a, b;
	uint32_t cnt, ecnt;
	...
	printf("%d vertices, %d edges, %d labels, %d label-node pairs\n", cnt, ecnt, uint32_t(g.labelnames.size()), uint32_t(g.links.size()));
}



/************** read edges **********/
void read_edges(graph & g, FILE *f)
{
	uint32_t a, b;
	uint32_t i = 0;
	...
	while (fscanf(f, "%d%d", &a, &b) == 2) {
		if (a == b) continue;
		g.edges[i].attach(&g.nodes[g.nodenames[a]], &g.nodes[g.nodenames[b]]);
		i++;
	}
}
...
/************** read **********/
void read(graph & g, FILE *lf, FILE *ef)
{
	read_sizes(g, lf, ef);
	rewind(lf);
	rewind(ef);
...
void buildtree(graph &g, FILE *out) {
    ...
    double total_possible_edges = (double)root->nodecnt*((root->nodecnt)-1)/2.0;
	double actual_edges = (double) g.edges.size();
    double root_score = (1.0-g.lambda)*(total_possible_edges-actual_edges);
...
int main(int argc, char **argv)
{
	...
	int ch;
	while ((ch = getopt_long(argc, argv, "ho:l:e:t:b:", longopts, NULL)) != -1) {
	switch (ch) {
		case 'h':
			printf("Usage: %s -e <edge  file> -l <label file> [-k <cnt>] [-hb] [-t <tree file>] [-o <node file>]\n", argv[0]);
			printf("  -h    print this help\n");
			printf("  -e    edge input file\n");
	...
	if (edgename == NULL) {
		printf("edge file not set\n");
		return 2;
	}
	if (labelname == NULL) {
		printf("label file not set\n");
		return 3;
	}
    if (!lambda_set) { 
		printf("lambda value not set. Use -b <value>\n");
		return 4;
    }
	FILE *ef = fopen(edgename, "r");
